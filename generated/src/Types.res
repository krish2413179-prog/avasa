...
module Transaction = {
  @genType
  type t = {hash: string, from: option<Address.t>, to: option<Address.t>, value: bigint}

  let schema = S.object((s): t => {hash: s.field("hash", S.string), from: s.field("from", S.nullable(Address.schema)), to: s.field("to", S.nullable(Address.schema)), value: s.field("value", BigInt.nativeSchema)})
}
...
module AggregatedTransaction = {
  @genType
  type t = {from: option<Address.t>, hash: string, to: option<Address.t>, value: bigint}
}
